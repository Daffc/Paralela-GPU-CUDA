...
__global__ void resolveTabuleiro(unsigned int *resposta, int rainhas){
    ...
    if(i < rainhas * rainhas){
        ...
        tabuleiro[2] = i % rainhas;

        if(nao_valido(tabuleiro + 1, i % rainhas, 1))
            resposta[i] = 0;
        else
...
int main(int argc, char *argv[])
{
    ...
    cudaMalloc((void **)&respostas, rainhas * rainhas * sizeof(unsigned int));    
    ...
    resposta = thrust::reduce(thrust::device, t_respostas, t_respostas + (rainhas * rainhas), 0);
